            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    if (jsonData.length != 0) {
                        $.each(jsonData, function(index, val) {
                            let rolesHtml = '';
                            let roleClass = '';
                            let showCellHtml = '<a class="btn btn-sm btn-success btn-block" href="users/' + val.id + '" data-toggle="tooltip" title="@lang("usersmanagement.tooltips.show")">@lang("Show User")</a>';
                            let editCellHtml = '<a class="btn btn-sm btn-info btn-block" href="users/' + val.id + '/edit" data-toggle="tooltip" title="@lang("usersmanagement.tooltips.edit")">@lang("Edit User")</a>';
                    ...
                    usersCount.html(jsonData.length + " results matching search");
                    userPagination.hide();
                ...
                error: function (response, status, error) {
                    if (response.status === 422) {
                        resultsContainer.append(noResulsHtml);
                        usersCount.html(0 + " results matching search");
                        userPagination.hide();
                        cardTitle.html("Search Results");
                    };
                },
