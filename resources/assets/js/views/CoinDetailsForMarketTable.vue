...
<style>
    ...
    * {
        font-family: "Open Sans Regular", "Open Sans", "Helvetica Neue", Helvetica,
    ...
    .market-cap-details ul li span,
    .market-cap-details ul li small {
        color: var(--secondary);
    }
...
<script>
    ...
    export default {
        name: "CoinDetailsForMarketTable",
        props: ["coincontent", "transaction"],
        data() {
            return {
                user_id: 0,
                chartRequest: null,
        ...
        methods: {
            ...
            chartDisplay: function () {
                ...
                self.chartRequest = $.ajax({
                    ...
                    success: function (response, status, error) {
                        ...
                        clsSign =
                            isNaN(response.updown) || response.updown == 0 ? "" : clsSign;

                        this.chartUpDown = {
                            updown: isNaN(response.updown)
                                ? ""
                                : Math.abs(numeral(response.updown).format("0.00")) + "%",
                            sign: clsSign
                        };
                            ...
                            xAxis: {
                                events: {
                                    setExtremes: function (event) {
                                        post_data.start_date = numeral(event.min)
                                            .divide(1000)
                                            .value();
                                        post_data.end_date = numeral(event.max)
                                            .divide(1000)
                                            .value();

                                        if (
                                            event.hasOwnProperty("DOMEvent") &&
                                            event.DOMEvent.type !== "mouseup"
                                        ) {
                                            return false;
                                            ...
                                            success: function (response, status, error) {
                                                ...
                                                let clsSign =
                                                    response.updown < 0
                                                        ...
                                                        : "fa-arrow-circle-o-up text-success";
                                                clsSign =
                                                    isNaN(response.updown) || response.updown == 0
                                                        ? ""
                                                        : clsSign;
            ...
            getCoinDetails() {
                let data = {
                    coin_coin: this.transaction,
                    coin: this.coincontent
                };
                $.ajax({
                    type: "GET",
